...
- File a phase as three primitives: `lc new item "<title>" --parent <theme> [--goal]` creates the item, `lc attach <item> brief <brief>` attaches the brief and `lc attach <item> repo <owner/name>` names the target repo by its registered identity (resolved through the project registry - a bare name works if it unambiguously matches one; an absolute path is used as-is), and `lc set <item> --state active [--workflow <origin/name>]` activates it - filing its workflow's entry step and handing it to the pipeline. Gate one step on another with `lc dep <step> --needs <id>`. Activation files the workflow's entry step and checks its declared `requires` (e.g. spec-driven's entry needs a `brief` and a `repo`, so it refuses an item missing either - attach both before activating). The workflow comes from the item or an ancestor (usually the theme); there is no default, so activation refuses an item with no workflow anywhere.
